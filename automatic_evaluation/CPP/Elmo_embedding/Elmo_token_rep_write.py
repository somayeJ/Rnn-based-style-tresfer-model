...
def create_elmo_embeddings(elmo, documents,suff, seq_embedding=False,max_sentences = 0):
    ...
    for elmo_embedding,tokens_list in zip(elmo.embed_sentences(tokens),tokens):
        ...
        if seq_embedding:
            ...
            # of embeddings of the tokens of each sequence with the shape 1024, across the sotoons, so the seq emb.shape of each sequence is 3072
        else:
        	if suff == 'tsf0':
        		...
        		word_list_file_style_orig0 =  "../opinion-lexicon-English/words-cleaned." + '0' # 0: negative
        		remover_class_orig0 = StyleMarkerRemover_elmo( "../../../../data/yelp/sentiment.test.0",word_list_file_style_orig0)
        		kept_tokens_indecies_orig0 = remover_class_orig0.get_all_content_sequences()
        	elif suff =='yelp1':

        		word_list_file_style_orig =  "../opinion-lexicon-English/words-cleaned." + '1' # 0: negative
        		remover_class_orig1 = StyleMarkerRemover_elmo( "../../../../data/yelp/sentiment.test.1",word_list_file_style_orig1)
        		kept_tokens_indecies_orig1 = remover_class_orig1.get_all_content_sequences()
...
#write_elmo_file(directory_write_yelp+".elmo.0", yelp0_elmo)
#write_elmo_file(directory_write_yelp+".elmo.1", yelp1_elmo)
