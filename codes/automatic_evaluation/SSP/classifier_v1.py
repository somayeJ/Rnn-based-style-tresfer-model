# me: adding the makeup method to balance the data when training, alan in file balance mikone data ro
# me: adding the confusion matrix
import os
import sys
import time
import ipdb
...
def evaluate(sess, args, vocab, model, x, y):
    ...
    if args.test:
        ...
        print('sum(same), len(y)',sum(same), len(y))
...
def prepare(path, args, suffix='tsf'):
    '''
    :param path:
    :param suffix:
    ...
    if args.model_type == 'transformer':
        data0 = load_sent(path + '.' + suffix  + '.0' + '.txt') # data0 aval - boode , tu style transfer files yani .tsf files + hast
        data1 = load_sent(path + '.' + suffix  + '.1' + '.txt')
    '''
    data0 = load_sent(path + '.0.' + suffix) # data0 aval - boode , tu style transfer files yani .tsf files + hast
