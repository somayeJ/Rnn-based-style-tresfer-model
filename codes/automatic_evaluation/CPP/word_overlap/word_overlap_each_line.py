...
def word_overlap_score_evaluator(source_file_path, target_file_path, source_style, target_style,scores_file_path):
    ...
    for word_list_1, word_list_2 in zip(actual_word_lists, generated_word_lists):
        ...
        if word_union:
            ...
            scores.append(score)
            #print(score, '--------------------------------------------------------------')
    with open(scores_file_path,'w') as fw:
        for score in scores:
            fw.write(str(score))
            fw.write('\n')
...
def main(argv):
    #print(argv)
    word_overlap_score = word_overlap_score_evaluator(argv[0],argv[1],argv[2],argv[3],argv[4]) # source_file_path, target_file_path, source_style, target_style
