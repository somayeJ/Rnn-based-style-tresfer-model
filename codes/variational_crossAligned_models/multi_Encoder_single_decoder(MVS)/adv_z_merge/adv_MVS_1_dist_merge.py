...
class Model(object):
    ...
    def __init__(self, args, vocab):
        ...
        #    dtype=tf.float32, scope='encoder')
        z = tf.concat([z0,z1],0)
        print('zzzzzzzzzzzzzzzzzzzzzz',z)
        ...
        self.h_ori0 = tf.concat([linear(labels[:half], dim_y,
            scope='generator'), self.z0], 1)
        self.h_tsf0 = tf.concat([linear(1-labels[:half], dim_y,
            scope='generator', reuse=True), self.z0], 1)
        ...
        self.h_ori = tf.concat([self.h_ori0,self.h_ori1],0)
        self.h_tsf = tf.concat([self.h_tsf0, self.h_tsf1],0)

        print('h_ori', self.h_ori)
        print('h_tsf', self.h_tsf)
        '''
        ...
        g_outputs = tf.concat([g_outputs, tf.tile(tf.reshape(self.z, [self.batch_size, 1, dim_z]), [1, self.len, 1])],  axis=2)  # in reshaping the product of the dimensions (SIZE) must remain the same as that of the original shape
        ...
        self.loss_rec  = loss_rec + (landa *divergence)
        ...
        soft_func = softsample_word(self.z,self.dropout, proj_W, proj_b, embedding,
            self.gamma)
        ...
        hard_func = argmax_word(self.z,self.dropout, proj_W, proj_b, embedding)
        ...
        self.loss_d0, self.loss_g0 = discriminator(teach_h[:half], soft_h_tsf[half:],
            ones, zeros, filter_sizes, n_filters, self.dropout,
            scope='discriminator0')
        self.loss_d1, self.loss_g1 = discriminator(teach_h[half:], soft_h_tsf[:half],
            ones, zeros, filter_sizes, n_filters, self.dropout,
        ...
        theta_eg = retrive_var(['encoder0', 'encoder1', 'generator',
            'embedding', 'projection'])
        theta_e0g = retrive_var(['encoder0',  'generator',
            'embedding', 'projection'])
        theta_e1g = retrive_var(['encoder1', 'generator',
            'embedding', 'projection'])
        ...
        grad_rec0, _ = zip(*opt.compute_gradients(self.loss_rec0, theta_e0g))
        grad_rec1, _ = zip(*opt.compute_gradients(self.loss_rec1, theta_e1g))
        grad_adv0, _ = zip(*opt.compute_gradients(self.loss_g0, theta_e1g))
        grad_adv1, _ = zip(*opt.compute_gradients(self.loss_g1, theta_e0g))

        grad0, _ = zip(*opt.compute_gradients(self.loss0, theta_eg))
        grad0, _ = tf.clip_by_global_norm(grad0, grad_clip)
        grad1, _ = zip(*opt.compute_gradients(self.loss1, theta_eg))
        ...
        self.optimize_rec0 = opt.minimize(self.loss_rec0, var_list=theta_e0g)
        self.optimize_rec1 = opt.minimize(self.loss_rec1, var_list=theta_e1g)

        self.optimize_tot0 = opt.apply_gradients(zip(grad0, theta_eg))
        self.optimize_tot1 = opt.apply_gradients(zip(grad1, theta_eg))
            ...
            for epoch in range(1, 1+args.max_epochs):
                ...
                for batch in batches:
                    ...
                    if loss_d0 < 1.2 and loss_d1 < 1.2:
                        optimize = model.optimize_tot0
                        optimize = model.optimize_tot1
                    else:
                        optimize = model.optimize_rec0
                        optimize = model.optimize_rec1
