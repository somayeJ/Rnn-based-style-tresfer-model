# !/usr/bin/python
# coding=utf-8
# in this model, the difference with style_transfer.py is that it adds the KL regularization as well, so the model is VAE_gan
import os
import sys
import time
import ipdb
...
class Model(object):
    ...
    def __init__(self, args, vocab):
        ...
        loc = tf.layers.dense(z,dim_z) #tf.layers.dense: adds one layer to the network, default activation: linear, unit:dim_z( dimensionality of the output space.)
        scale = tf.layers.dense(z, dim_z, tf.nn.softplus)
        posterior = tfd.MultivariateNormalDiag(loc, scale) # Gaussian with diagonal standard deviation

        def make_prior(code_size):
            loc = tf.zeros(code_size)
            scale = tf.ones(code_size)
            mvn = tfd.MultivariateNormalDiag(loc, scale) #mvn.stddev().eval()= scale, mvn.mean.eval() = loc
            #return tfd.MultivariateNormalDiag(loc, scale) 
            return mvn

        prior = make_prior(dim_z) # it is a distribution
        z = posterior.sample()    
        ...
        # after concat, self.h_ori.shape = batch_size * 1 * dim_y + dim_z + dim_g_outputs
        ...
        divergence = tfd.kl_divergence(posterior, prior)# Our goal is to find the variational parameters \lambdaλ that minimize this divergence
        #(if q were Gaussian,lambdaλ would be the mean and variance of the latent variables for each datapoint ) . 
        divergence =  tf.reduce_mean(divergence) / tf.to_float(self.batch_size)
        #print('***',type(divergence))
        self.elbo = loss_rec + (landa *divergence)
        #print('***',type(self.elbo))
        ...
        # we produce to the no of max_len the size can be produced after the token ens (they are not pads at the end)
        ...
        self.loss = self.elbo + self.rho * self.loss_adv
...
def transfer(model, decoder, sess, args, vocab, data0, data1, out_path):
    ...
    for batch in batches:
        ...
        loss, elbo, loss_adv, loss_d0, loss_d1 = sess.run([model.loss,
            model.elbo, model.loss_adv, model.loss_d0, model.loss_d1],
            feed_dict=feed_dictionary(model, batch, args.rho, args.gamma_min))
        losses.add([loss, elbo, loss_adv, loss_d0, loss_d1])
        ...
        if args.train:
            ...
            for epoch in range(1, 1+args.max_epochs):
                ...
                for batch in batches:
                    ...
                    loss, elbo, loss_adv, _ = sess.run([model.loss,
                        model.elbo, model.loss_adv, optimize],
                        feed_dict=feed_dict)
                    losses.add([loss, elbo, loss_adv, loss_d0, loss_d1])
