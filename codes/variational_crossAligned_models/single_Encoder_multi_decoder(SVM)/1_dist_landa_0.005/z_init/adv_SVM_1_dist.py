...
class Model(object):
    def __init__(self, args, vocab):
        ...
        print('divergence0',divergence)
        divergence =  tf.reduce_mean(divergence) / tf.to_float(self.batch_size)
        print('divergence0',divergence)
        self.elbo0 = loss_rec0 + (landa * divergence)
        ...
        divergence = tfd.kl_divergence(posterior, prior)
        print('divergence1',divergence)
        # for VAE it is good to have tf.reduce_mean & not tf.reduce_sum
        divergence =  tf.reduce_mean(divergence) / tf.to_float(self.batch_size)
        print('divergence1',divergence)

        self.elbo1 = loss_rec1 + (landa * divergence)
        ...
        soft_func0 = softsample_word(self.dropout, proj_W0, proj_b0, self.embedding,
            self.gamma)
        soft_func1 = softsample_word(self.dropout, proj_W1, proj_b1, self.embedding,
            self.gamma)
        ...
        hard_func0 = argmax_word(self.dropout, proj_W0, proj_b0, self.embedding)
        hard_func1 = argmax_word(self.dropout, proj_W1, proj_b1, self.embedding)
        ...
        if args.train:
            ...
            for epoch in range(1, 1+args.max_epochs):
                ...
                for batch in batches:
                    ...
                    loss, elbo, loss_adv, _ ,_= sess.run([model.loss,
                        model.elbo, model.loss_adv, optimize0, optimize1],
                        feed_dict=feed_dict)
                    losses.add([loss, elbo, loss_adv, loss_d0, loss_d1])
