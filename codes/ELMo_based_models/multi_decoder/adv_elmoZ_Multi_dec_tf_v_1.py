# This code is to to do the style transfer task by feeding the decoder by elmo rep of seqs and training the corresponding losses of the discriminator and decoder
import os
import sys
import time
import ipdb
...
class Model(object):
    def __init__(self, args, vocab):
        # y: style
        elmo_seq_rep = args.elmo_seq_rep # elmo- rep - directory
        ...
        init_state = tf.concat([linear(labels, dim_y, scope='encoder'),
            tf.zeros([self.batch_size, 3072])], 1) # Tensor("concat:0", shape=(?, 700), dtype=float32) 700= dim_h = dim y + dim_z
        print(init_state)
        # create GRU cell
        ...
        self.h_ori0 = tf.concat([linear(labels[:half], dim_y,
            scope='generator0'), self.elmo_emb[:half]], 1) #rx0
        self.h_ori1 = tf.concat([linear(labels[half:], dim_y,
            scope='generator1'), self.elmo_emb[half:]], 1) #rx1

        self.h_tsf0 = tf.concat([linear(1-labels[:half], dim_y,
            scope='generator1', reuse=True), self.elmo_emb[:half]], 1) #tsf0
        self.h_tsf1 = tf.concat([linear(1-labels[half:], dim_y,
            scope='generator0', reuse=True), self.elmo_emb[half:]], 1) #tsf1
        ...
        # Tensor("concat_3:0", shape=(?, ?, 700), dtype=float32) # after concat, teach_h.shape = batch_size * 1 + dim_g_outputs(max_len + 1) * dim_y + dim_z
...
def transfer(model, decoder, sess, args, vocab, data0, data1, out_path):
    if args.dev:
        ...
        batches, order0, order1 = get_batches_elmo(data0, data1,
            vocab.word2id, args.batch_size, ".test.", args.elmo_seq_rep) # sentiment.dev.elmo.0
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.beam > 1:
            decoder = beam_search_tf_1.Decoder_multi_dec(sess, args, vocab, model)
        else:
