# this code is to create the embeddings as the last state of the encoder for probing classification experiments, and takes train, dev  or test data and with two styles saves them into files with their numpy format
# it can be run in the terminal by running a code like the following (for saving emb of train data):
...
class Model(object):
    def __init__(self, args, vocab):
        ...
        self.h_ori0 = tf.concat([linear(labels[:half], dim_y,
            scope='generator0'), self.z[:half]], 1) #rx0
        self.h_ori1 = tf.concat([linear(labels[half:], dim_y,
            scope='generator1'), self.z[half:]], 1) #rx1

        self.h_tsf0 = tf.concat([linear(1-labels[:half], dim_y,
            scope='generator1', reuse=True), self.z[:half]], 1) #tsf0
        self.h_tsf1 = tf.concat([linear(1-labels[half:], dim_y,
            scope='generator0', reuse=True), self.z[half:]], 1) #tsf1
        ...
        print('teach0',teach_h0) # Tensor("concat_3:0", shape=(?, ?, 700), dtype=float32) # after concat, teach_h.shape = batch_size * 1 + dim_g_outputs(max_len + 1) * dim_y + dim_z
        print('teach1', teach_h1)
        g_outputs0 = tf.nn.dropout(g_outputs0, self.dropout)
        print('z000000', self.z[:half])
        g_outputs0 = tf.concat([g_outputs0, tf.tile(tf.reshape(self.z[:half], [self.batch_size/2, 1, dim_z]), [1, self.len, 1])],  axis=2)  # in reshaping the product of the dimensions (SIZE) must remain the same as that of the original shape
        ...
        print('z1111', self.z[half:])

        g_outputs1 = tf.concat([g_outputs1, tf.tile(tf.reshape(self.z[half:], [self.batch_size/2, 1, dim_z]), [1, self.len, 1])],  axis=2)  # in reshaping the product of the dimensions (SIZE) must remain the same as that of the original shape
        print('g_outputs1',g_outputs1)
        ...
        soft_func0 = softsample_word(self.z[:half],self.dropout, proj_W0, proj_b0, self.embedding,
            self.gamma)
        soft_func1 = softsample_word(self.z[half:],self.dropout, proj_W1, proj_b1, self.embedding,
            self.gamma)
        ...
        hard_func0 = argmax_word(self.z[:half],self.dropout, proj_W0, proj_b0, self.embedding)
        hard_func1 = argmax_word(self.z[half:],self.dropout, proj_W1, proj_b1, self.embedding)
