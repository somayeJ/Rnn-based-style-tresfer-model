# In this code, conditions of checking the performance of the two classifiers are added before starting the back propagation
# multi_decoder_condition_2.py, compared to multi_decoder_condition.py, it removes style parts and also back propagates
# the adv_loss only on generator
import os
import sys
import time
import ipdb
...
class Model(object):
    def __init__(self, args, vocab):
        ...
        self.h_ori0 = tf.concat([linear(labels[:half], dim_y,
            scope='generator0'), z[:half]], 1) #rx0
        self.h_ori1 = tf.concat([linear(labels[half:], dim_y,
            scope='generator1'), z[half:]], 1) #rx1
        ...
        self.loss0 = self.rho * self.loss_g0
        self.loss1 = self.rho * self.loss_g1
        self.loss = self.loss0 + self.loss1 +self.loss_rec
        ...
        theta_g0 = retrive_var([ 'generator0',
            'projection0'])
        theta_g1 = retrive_var([ 'generator1',
             'projection1'])
        ...
        grad0, _ = zip(*opt.compute_gradients(self.loss0, theta_g0))
        grad0, _ = tf.clip_by_global_norm(grad0, grad_clip)
        grad1, _ = zip(*opt.compute_gradients(self.loss1, theta_g1))
        ...
        self.optimize_tot0 = opt.apply_gradients(zip(grad0, theta_g0))
        self.optimize_tot1 = opt.apply_gradients(zip(grad1, theta_g1))
