...
def softmax_word(dropout, proj_W, proj_b, embedding, gamma):
    ...
    def loop_func(output):
        ...
        log_prob = tf.nn.log_softmax(logits / gamma)
        #inp = tf.matmul(prob, embedding)
        inp = tf.matmul(tf.math.exp(log_prob) , embedding)
        return inp, logits, prob, log_prob
...
def argmax_word(dropout, proj_W, proj_b, embedding):
    ...
    def loop_func(output):
        ...
        word = tf.argmax(log_prob, axis=1)
        inp = tf.nn.embedding_lookup(embedding, word)
        #return word, inp, logits
        return inp, logits, word, log_prob

    return loop_func
#rnn_decode(self.h_tsf, go, max_len,cell_g,  softsample_word, scope='generator')   
#
def rnn_decode(h, inp, length, cell, loop_func, scope):
    gen_token_seq, h_seq, logits_seq, probs_seq, log_probs = [], [], [], [], []
    inp_shape = inp.shape
    ...
    with tf.variable_scope(scope):
        ...
        for t in range(length):
            ...
            output, h = cell(inp, h)
            inp, logits, prob, log_prob  = loop_func(output)
            inp.set_shape(inp_shape)
            logits_seq.append(tf.expand_dims(logits, 1))
            gen_token_seq.append(tf.expand_dims(inp,1))
            probs_seq.append(tf.expand_dims(prob,1))
            log_probs.append(tf.expand_dims(log_prob,1))

    return tf.concat(gen_token_seq,1), tf.concat(h_seq, 1), tf.concat(logits_seq, 1), tf.concat(probs_seq,1), tf.concat(log_probs,1)
