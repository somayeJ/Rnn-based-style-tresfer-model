...
def load_arguments():
    ...
    #argparser.add_argument('--dropout_keep_prob',             type=float,            default=0) # transformer
    ...
    #argparser.add_argument('--dim_emb',             type=int,             default=500)
    argparser.add_argument('--learning_rate',            type=float,            default=0.0005)
    #argparser.add_argument('--learning_rate',             type=float,             default=0.0001) # transformer
    argparser.add_argument('--extra_features',
            type=bool,
            default=False)
    argparser.add_argument('--elmo_seq_rep',
            type=str,
            default='')
    argparser.add_argument('--dim_token_feat',
            type=int,
            default=0) # 6 ['like_url', 'is_alpha', 'is_stop', 'ent_type', 'dep']
    ...
    argparser.add_argument('--rho',                 # loss_rec + rho * loss_adv
            type=float,
            default=1)
    argparser.add_argument('--rev_tokens', 
            type=bool,
            default=False)
    argparser.add_argument('--cyclic_soft', 
            type=bool,
            default=True)

    argparser.add_argument('--cycl_factor',                
            type=float,
            default= 0.5)
    argparser.add_argument('--rec_factor',                
            type=float,
            default= 1) # 0.25 in trnsfrmr model
    ...
    argparser.add_argument('--keep_data_order',
            type=bool,
            default=False)
    argparser.add_argument('--downsampling_train_SDS',
            type=bool,
            default=False)# downsample the the bigger-sized dataset to train the models
