...
def feed_dictionary(model, batch, rho, rec_fac,gamma, dropout=1, learning_rate=None):
...
def noise(x, unk, shuffle, word_drop, k=3):
    n = len(x)
    for i in range(n):
        if random.random() < word_drop: # returns a random floating number between 0 and 1.
            x[i] = unk
    if not(shuffle):
        return x
    # this part of the code shuffles the data ,# slight shuffle such that |sigma[i]-i| <= k
...
def get_batch(x, y, word2id, args,  noisy=False,min_len=5):
    ...
    for sent in x:
        ...
        _sent_id = noise(sent_id, unk, args.shuffle_tokens, args.word_drop) 
        if args.rev_tokens:
