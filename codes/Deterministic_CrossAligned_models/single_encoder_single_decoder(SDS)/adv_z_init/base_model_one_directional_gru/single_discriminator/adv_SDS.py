...
class Model(object):
    def __init__(self, args, vocab):
        ...
        self.rho = tf.placeholder(tf.float32,
            name='rho')
        self.rec_fac = tf.placeholder(tf.float32,
            name='rec_fac')
        ...
        zeros0, ones0 = self.labels[:half], self.labels[half:]
        zeros = tf.concat([zeros0, zeros0], 0)
        ones = tf.concat([ones0, ones0] , 0)
        ...
        '''
        self.loss_d, self.loss_adv = discriminator(teach_h, soft_h_tsf,
            ones, zeros, filter_sizes, n_filters, self.dropout,
            scope='discriminator')        
        #####   optimizer   #####
        #self.loss_adv = loss_g0 + loss_g1
        self.loss = (self.rec_fac * self.loss_rec ) + (self.rho * self.loss_adv)
        ...
        #print(111111111111,self.optimize_rec)
...
def transfer(model, decoder, sess, args, vocab, data0, data1, out_path):
    ...
    losses = Accumulator(len(batches), ['loss', 'rec', 'adv', 'd'])
    for batch in batches:
        ...
        loss, loss_rec, loss_adv, loss_d = sess.run([model.loss,
            model.loss_rec, model.loss_adv, model.loss_d],
            feed_dict=feed_dictionary(model, batch, args.rho, args.rec_fac, args.gamma_min))
        losses.add([loss, loss_rec, loss_adv, loss_d])
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.train:
            ...
            for epoch in range(1, 1+args.max_epochs):
                ...
                for batch in batches:
                    feed_dict = feed_dictionary(model, batch, rho,rec_fac, gamma,
                    ...
                    loss_d1, _ = sess.run([model.loss_d1, model.optimize_d1],
                        feed_dict=feed_dict)
                    '''
                    loss_d, _ = sess.run([model.loss_d, model.optimize_d],
                        feed_dict=feed_dict)
