...
class Model(object):
    def __init__(self, args, vocab):
        ...
        token_seq_soft_ori, soft_h_ori, soft_logits_ori, soft_probs_ori = rnn_decode(self.h_ori, go, max_len,
        ...
        token_seq_soft_tsf, soft_h_tsf, soft_logits_tsf, soft_probs_tsf = rnn_decode(self.h_tsf, go, max_len,
            cell_g, soft_func, scope='generator')
        token_seq_hard_ori, hard_h_ori, self.hard_logits_ori, words_ori = rnn_decode(self.h_ori, go, max_len,
            cell_g, hard_func, scope='generator')
        token_seq_hard_tsf, hard_h_tsf, self.hard_logits_tsf, words_tsf  = rnn_decode(self.h_tsf, go, max_len,
            cell_g, hard_func, scope='generator')
        ...
        if cyclic_soft:
            _, z_cyc = tf.nn.dynamic_rnn(cell_e, soft_probs_tsf ,
                initial_state = init_state_tsf , scope='encoder') 
        else:
            _, z_cyc = tf.nn.dynamic_rnn(cell_e, words_tsf,
               initial_state = init_state_tsf , scope='encoder') 
