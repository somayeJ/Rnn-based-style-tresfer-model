...
class Model(object):
    def __init__(self, args, vocab):
        ...
        src_contexts0 = tf.concat([src_hs_fw, src_hs_bw],axis=2)
        print('src_contexts0',src_contexts0)
        src_contexts = tf.concat([src_hs_fw[:,:,100:], src_hs_bw[:,:,100:]],axis=2)
        # src_contexts= [batchsize, batch_len_input, 2*250]

        self.z = tf.concat([src_h_fw[:,100:], src_h_bw[:,100:]] , axis=1)

        #cell_e = create_cell(dim_h, n_layers, self.dropout) # dim_h = dim_y +dim_z =700
        # _, z2 = tf.nn.dynamic_rnn(cell_e, enc_inputs,    initial_state=init_state2, scope='encoder') # z = last hidden layer of the encoder, z.shape: (batch_size, dim_h)
        #print(z)
        #print(z2.shape())

        # for z, we consider, just the z part and reduce the style part
        #z = z[:, dim_y:]
        #print(z)  # print(z): Tensor("strided_slice:0", shape=(?, 500), dtype=float32)
        # cell_e = create_cell(dim_z, n_layers, self.dropout)
        # _, z = tf.nn.dynamic_rnn(cell_e, enc_inputs,
        #    dtype=tf.float32, scope='encoder')
        
        '''
        self.h_ori0 = tf.concat([linear(labels, dim_y,
            scope='generator'), self.z], 1)
        self.h_tsf0 = tf.concat([linear(1-labels, dim_y,
            scope='generator', reuse=True), self.z], 1)
        '''
        #1: change init_state decoder
        #init_state_g = tf.concat([linear(labels, dim_y, scope='generator'),
           # tf.zeros([self.batch_size, dim_z])], 1) # Tensor("concat:0", shape=(?, 700), dtype=float32) 700= dim_h = dim y + dim_z
        ...
        g_outputs, g_state = tf.nn.dynamic_rnn(cell_g, dec_inputs,
            initial_state=self.h_ori, scope='generator')
        #g_outputs_size = [batch_size, batch_len, dim_h] 
        print('g_outputs',g_outputs) # Tensor("generator_2/transpose_1:0", shape=(?, ?, 700), dtype=float32) # shape= batch_size* max_len * dim_h
        ...
        #2. add this part
        self.expanded_tgt = tf.tile(tf.reshape(g_outputs, [ self.batch_size, self.batch_len, 1, dim_h]), [ 1, 1, self.batch_len_input, 1 ])
        # self.expanded_tgt.shape: [batch_size, self.batch_len,batch_len_input, dim_h]
        self.expanded_src = tf.tile(tf.reshape(src_contexts, [ self.batch_size, 1,self.batch_len_input, 2*250]), [ 1, self.batch_len, 1, 1 ])
        # self.expanded_src.shape: [batch_size,self.batch_len, batch_len_input, 2*250]
        score_input = tf.concat([
            tf.reshape(self.expanded_tgt, [ self.batch_size, self.batch_len*self.batch_len_input, dim_h ]),
            tf.reshape(self.expanded_src, [ self.batch_size, self.batch_len_input*self.batch_len, 2*250 ])], axis=2)
        # score_input.shape: [batch_size,max_len* batch_len, 2*250+dim_h]
        ...
        scores_h = tf.tanh(tf.matmul(tf.reshape(score_input, [ self.batch_size*self.batch_len*self.batch_len_input, dim_h+2*250 ]), W_score1) + b_score1)
        #score_h.shape: [batch_size*max_len* batch_len, att_hidden_size]
        #scores   = tf.reshape(tf.matmul(scores_h, W_score2), [ batch_size, max_len, self.batch_len ])
        scores = tf.reshape(tf.matmul(scores_h, W_score2), [ self.batch_size* self.batch_len* self.batch_len_input ])
        #score.shape: [batch_size*self.batch_len* self.batch_len_input ]
        #src_mask = tf.tile(tf.reshape(tf.sequence_mask(src_lens, src_num_steps, tf.float32), [ batch_size, 1, src_num_steps ]), [ 1, tgt_num_steps, 1 ])
        print('weights_input_shape',self.weights_input.shape)
        print('self.batch_size',self.batch_size.shape)
        print('self.batch_len',self.batch_len.shape)
        
        src_mask = tf.tile(tf.reshape(self.weights_input, [ self.batch_size, 1, self.batch_len_input ]), [ 1, self.batch_len, 1 ])
        #src_mask.shape =  [batch_size,batch_len, batch_len_input]
        #scores   = scores*src_mask + -1e20*(1.0 - src_mask)
        scores *= tf.reshape(src_mask,[-1])
        scores = tf.reshape(scores,[ self.batch_size, self.batch_len, self.batch_len_input])
        #scores.shape =  [batch_size,self.batch_len, batch_len_input]
        att = tf.nn.softmax(scores)

        expanded_att = tf.tile(tf.reshape(att, [ self.batch_size, self.batch_len, self.batch_len_input, 1]), [ 1, 1, 1, 2*250 ])
        # expanded_att.shape: [batch_size, self.batch_len, self.batch_len_input, 2*250]

        # this is the context vector
        self.h = tf.reduce_sum(self.expanded_src*expanded_att, axis=2)
        # h.shape: [batch_size,self.batch_len,  2*250]

        #g_outputs_shape = [batch_size, self.batch_len, dim_h]
        teach_h = tf.concat([tf.expand_dims(self.h_ori, 1), g_outputs], 1)
        #teach_h = tf.concat([self.h,  g_outputs],  axis=2)
        #teach_h.shape = [batch_size, max_len+1, dim_h]
        ...
        print('181',teach_h) # Tensor("concat_3:0", shape=(?, ?, 700), dtype=float32) # after concat, teach_h.shape = batch_size * 1 + dim_g_outputs(max_len + 1) * dim_y + dim_z
        #teach_h = tf.concat([tf.expand_dims(self.h_ori, 1), tf.concat([ g_outputs, h ], axis=2)], 1)
        #teach_h = tf.concat([ g_outputs, self.h ], axis=2) 
        # # teach_h.shape = [batch_size, max_len, dim_h+ (2*250)]

        g_outputs = tf.nn.dropout(g_outputs, self.dropout)
        print('187',g_outputs.shape)
        #g_outputs_size = [batch_size, max_len, dim_h] 
        preoutput = tf.concat([ g_outputs, self.h ], axis=2)
        print('190',self.h.shape)
        print('191',preoutput.shape)

        #preoutput.shape = [batch_size , max_len, dim_h+(2*250)]

        self.preoutput = tf.reshape(preoutput,[-1, dim_h+(2*250)])
        g_logits = tf.matmul(self.preoutput, proj_W) + proj_b
        ...
        ####
        #  soft_h_tsf estefade mishe sar zama e train
        # self.hard_logits_ori && self.hard_logits_tsf: dar decoder.rewrite dar tabee transfer estefae mishe, yani dar zamane inference
        # 
        ####
        self.src_contexts = tf.tile(tf.reshape(src_contexts, [ self.batch_size, 1,self.batch_len_input, 2*250]), [ 1, 1, 1, 1 ])
        # self.expanded_src.shape: [batch_size,self.batch_len, batch_len_input, 2*250]
        #src_contexts.shape: [batchsize, batch_len_input, 2*250]
        soft_h_ori, soft_logits_ori = rnn_decode(dim_h,self.batch_size,W_score1,b_score1,W_score2,self.src_contexts,self.h_ori, go, self.weights_input,self.batch_len_input,max_len,
            cell_g, soft_func, scope='generator')
        # x_fake
        soft_h_tsf, soft_logits_tsf = rnn_decode(dim_h,self.batch_size,W_score1,b_score1,W_score2,self.src_contexts,self.h_tsf, go, self.weights_input,self.batch_len_input,max_len,
            cell_g, soft_func, scope='generator')
        hard_h_ori, self.hard_logits_ori = rnn_decode(dim_h,self.batch_size,W_score1,b_score1,W_score2,self.src_contexts,self.h_ori, go, self.weights_input,self.batch_len_input,max_len,
            cell_g, hard_func, scope='generator')
        hard_h_tsf, self.hard_logits_tsf = rnn_decode(dim_h,self.batch_size,W_score1,b_score1,W_score2,self.src_contexts,self.h_tsf, go,self.weights_input,self.batch_len_input, max_len,
            cell_g, hard_func, scope='generator')
