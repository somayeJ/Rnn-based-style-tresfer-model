...
class Decoder(object):
    ...
    def __init__(self, sess, args, vocab, model):
        ...
        with tf.variable_scope('generator'):
            ...
            expanded_query1=tf.reshape(tf.matmul(tf.reshape(expanded_query, [ self.batch_size*1, dim_h ]), W_score1) + b_score1,[self.batch_size,1, att_hidden_size])
            src_context1=tf.reshape(tf.matmul(tf.reshape(self.src_contexts, [ self.batch_size*self.batch_len_input, dim_h ]), W_score2) + b_score2,[self.batch_size, self.batch_len_input, att_hidden_size])

            pre_scores = tf.tanh(expanded_query1+src_context1)
            score_h = tf.matmul(tf.reshape(pre_scores, [self.batch_size*self.batch_len_input, att_hidden_size]),W_score3)+b_score3
    ...
    def rewrite(self, batch):
        model = self.model
        h_ori, h_tsf ,src_contexts,z= self.sess.run([model.h_ori, model.h_tsf,model.src_contexts0, model.z0],
        ...
        ori = self.decode(h_ori,batch,src_contexts,z)
        ori = [[self.vocab.id2word[i] for i in sent] for sent in ori]
        ori = strip_eos(ori)
        tsf = self.decode(h_tsf,batch,src_contexts,z)
