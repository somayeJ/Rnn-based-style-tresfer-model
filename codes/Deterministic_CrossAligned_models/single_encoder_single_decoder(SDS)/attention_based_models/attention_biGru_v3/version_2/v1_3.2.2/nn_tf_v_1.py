...
def attention(dim_h,batch_size,W_score1,b_score1,W_score2,b_score2,W_score3,output,weights_input,batch_len_input,src_context):
    # expand tgt 
    ...
    #1: 
    expanded_query1=tf.reshape(tf.matmul(tf.reshape(expanded_query, [ batch_size*1, dim_h ]), W_score1) + b_score1,[batch_size,1, att_hidden_size])
    src_context1=tf.reshape(tf.matmul(tf.reshape(src_context, [ batch_size*batch_len_input, dim_h ]), W_score2) + b_score2,[batch_size, batch_len_input, att_hidden_size])
...
def rnn_decode(mode,z,dim_h,batch_size,W_score1,b_score1,W_score2,b_score2,W_score3,src_context,h, inp, end, weights_input,batch_len_input,length, cell, loop_func, scope):
    ...
    with tf.variable_scope(scope):
        ...
        if mode=='':
            ...
            for t in range(length):
                ...
                attention_weights,context_vector0 = attention(dim_h,batch_size,W_score1,b_score1,W_score2,b_score2,W_score3,output,weights_input,batch_len_input,src_context)
            ...
            for t in range(length+1):
                ...
                attention_weights,context_vector = attention(dim_h,batch_size,W_score1,b_score1,W_score2,b_score2,W_score3,output,weights_input,batch_len_input,src_context)
                print('inp',inp)
                print('output',output)
                print(mode,t,'t')
