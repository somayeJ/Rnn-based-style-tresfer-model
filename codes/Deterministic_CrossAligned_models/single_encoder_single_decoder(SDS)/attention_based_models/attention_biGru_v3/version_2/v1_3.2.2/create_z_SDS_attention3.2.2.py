...
def cal_avg_context_vector(length,len_generated_outputs, context_vector_seq, attention_weight_seqs,real_len_input):
    ...
    # trg_mask:
    trg_mask = np.reshape(weights_generated_outputs,[batch_size,max_len_output,1])
    ...
    context_vector_seq1= np.reshape(np.sum(context_vector_seq0, axis=1),[batch_size,700])# shape: [batch_size,1, 700]   
    print('context_vector_seq1[1]',context_vector_seq1.shape,context_vector_seq1[1])
    
    average_context_vector = np.transpose(np.divide(np.transpose(context_vector_seq1),len_generated_outputs ))# shape: [batch_size, 700] 
    print('average_context_vector[1]',average_context_vector[1])
    print('average_context_vector[2]',average_context_vector[2])
    print('context_vector_seq1[2]',context_vector_seq1.shape,context_vector_seq1[2][680:])
    print('average_context_vector[2]',average_context_vector[2][680:])
    attention_weights_masked = attention_weight_seqs * trg_mask#  attention_weight_seqs.shape: batch_size, max_len_output, max_len_input, [128,31,31]
    print('attention_weight_seqs0; real_len_input,len_generated_outputs', real_len_input[0],len_generated_outputs[0],attention_weight_seqs[0])
    print('attention_weight_seqs1; real_len_input,len_generated_outputs', real_len_input[1],len_generated_outputs[1],attention_weight_seqs[1])
    print('attention_weight_seqs2',attention_weight_seqs.shape,real_len_input[2],len_generated_outputs[2],attention_weight_seqs[2])
    print('attention_weights_masked0',attention_weights_masked.shape,real_len_input[0],len_generated_outputs[0],attention_weights_masked[0])
    print('attention_weights_masked1',attention_weights_masked.shape,real_len_input[1],len_generated_outputs[1],attention_weights_masked[1])
    print('attention_weights_masked2',attention_weights_masked.shape,real_len_input[2],len_generated_outputs[2],attention_weights_masked[2])
    print('real_len_input',real_len_input[:10])
...
def seq_embeddings(model, sess, args, vocab, data0,data1):
    ...
    batches, order0, order1 = get_batches(data0,data1,vocab.word2id, args.batch_size, args)
    embed_data0, embed_data1, attention_w0,attention_w1 , embed_data00, embed_data10, attention_w00,attention_w10 ,all_gen_seq_len0    ,all_gen_seq_len1,sen0,sen1,  all_input_lens0, all_input_lens1=[],[],[],[], [], [], [] ,[], [], [], [] ,[], [], []
    for batch in batches:
        ...
        print('real_len_input',real_len_input[32:])
        ...
        print(batch['text_inputs'][32:])
        print(len_generated_seqs[32:])
        ...
        print('attention_weights_masked',attention_weights_masked[0][15])
        #print('trg_mask',trg_mask[0].eval())
        embed_data0.extend(avg_context_vector[:half, :])
        embed_data1.extend(avg_context_vector[half:, :])
        attention_w0.extend(attention_weights_masked[:half, :,:])
        attention_w1.extend(attention_weights_masked[half:,:,:])
        print(len(embed_data0),len(embed_data1),len(attention_w0),len(attention_w1))

        #embed_data0.extend(avg_context_vector[:half, :])
        #embed_data1.extend(avg_context_vector[half:,:])
        sen0.extend(batch['text_inputs'][:half])
        sen1.extend(batch['text_inputs'][half:])
        all_input_lens0.extend(real_len_input[:half])
        all_input_lens1.extend(real_len_input[half:])
        all_gen_seq_len0.extend(len_generated_seqs[:half])
        all_gen_seq_len1.extend(len_generated_seqs[half:])

    print("len(embed_data1),len(embed_data0), len(sen0), len(sen1) after loop",len(embed_data1),len(embed_data0), len(sen0), len(sen1) )
    return embed_data0, embed_data1, sen0, sen1, attention_w0, attention_w1,all_input_lens0, all_input_lens1,all_gen_seq_len0, all_gen_seq_len1
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        len_src0=np.load(args.output+'src_len.0'+'.npy',allow_pickle=False )
        len_src1=np.load(args.output+'src_len.1'+'.npy',allow_pickle=False )

        len_g0=np.load(args.output+'gen_len.0'+'.npy',allow_pickle=False )
        len_g1=np.load(args.output+'gen_len.1'+'.npy',allow_pickle=False )

        print('length of len(vectors0)',len(vectors0), len(vectors0[0]))
        print('length oflen(vectors1)',len(vectors1), len(vectors1[0]))

        print('length of len(len_src0)',len(len_src0))
        print('length of len(len_src1)',len(len_src1))

        print('length of len(len_g0)',len(len_g0))
        print('length of len(len_g1)',len(len_g1))

        print('length of len(vec0)',len(vec0), len(vec0[0]), vec0[0][0])
        print('length of len(vec1)',len(vec1), len(vec1[0]),vec1[0][0])
