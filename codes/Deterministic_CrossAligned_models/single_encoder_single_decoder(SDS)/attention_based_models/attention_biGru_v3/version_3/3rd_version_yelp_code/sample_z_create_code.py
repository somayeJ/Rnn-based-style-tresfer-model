...
# ezafe konim
def avg_context_vector(length,len_generated_outputs, context_vector_seq, attention_weight_seqs):
    weights_generated_outputs = []
    max_len_output =length
    for len_sent in len_outputs:
        w_each_seq=[]
        for i in range(len_sent):
            w_each_seq.append([1.0])
        for i in range(max_len_output-len_sent):
            w_each_seq.append([0.0])
        weights_generated_output.append(w_each_seq)
        # trg_mask:
        trg_mask = tf.reshape(weights_generated_output,[batch_size,max_len_output, 1 ])
        context_vector_seq0= context_vector_seq*trg_mask
        # here, we ignore the context vector correponding to the tokens that are masked
        context_vector_seq1= tf.reduce_sum(context_vector_seq0, 1)# shape: [batch_size,1, 700]   
        average_context_vector = tf.math.divide(context_vector_seq1,tf.float(len_sent)) 
        attention_weight_vecor_seqs_masked =   attention_weight_seqs * trg_mask
    return average_context_vector , attention_weights_masked
gen_tokens = np.argmax(tf.concat(logits_seq, 1), axis=2)
gen_tokens = [[self.vocab.id2word[i] for i in sent] for sent in gen_tokens]
