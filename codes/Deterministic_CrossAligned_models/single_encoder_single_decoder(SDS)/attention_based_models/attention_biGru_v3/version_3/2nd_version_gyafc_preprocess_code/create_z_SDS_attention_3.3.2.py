...
class Model(object):
    def __init__(self, args, vocab):
        ...
        soft_h_ori, self.soft_logits_ori ,self.context_vector_seq_sori, self.attention_w_seq_sori= rnn_decode('',self.z0,dim_h,self.batch_size,W_score1,W_score2,W_score3,
            self.src_contexts0,self.h_ori, go, end,self.weights_input,self.batch_len_input,max_len,cell_g, soft_func, scope='generator')
        # x_fake
        soft_h_tsf, self.soft_logits_tsf,self.context_vector_seq_stsf, self.attention_w_seq_stsf = rnn_decode('',self.z0,dim_h,self.batch_size,W_score1,W_score2,W_score3,
            self.src_contexts0,self.h_tsf, go, end,self.weights_input,self.batch_len_input,max_len,  cell_g, soft_func,scope='generator')
        
        hard_h_ori, self.hard_logits_ori,self.context_vector_seq_hori, self.attention_w_seq_hori = rnn_decode('',self.z0,dim_h,self.batch_size,W_score1,W_score2,W_score3,
            self.src_contexts0,self.h_ori, go, end,self.weights_input,self.batch_len_input,max_len,cell_g, hard_func, scope='generator')
        hard_h_tsf, self.hard_logits_tsf,self.context_vector_seq_htsf, self.attention_w_seq_htsf= rnn_decode('',self.z0,dim_h,self.batch_size,W_score1,W_score2,W_score3,
            self.src_contexts0,self.h_tsf, go,end,self.weights_input,self.batch_len_input, max_len, cell_g, hard_func, scope='generator')

        self.teach_h,self.g_logits0,self.context_vector_seq_teach0, self.attention_w_seq_teach0 = rnn_decode('teacher_force',self.z0,dim_h,self.batch_size,W_score1,W_score2,W_score3,
            self.src_contexts0,self.h_ori,go, end,self.weights_input, self.batch_len_input,max_len, cell_g,proj_func,scope='generator')
        print('g_logits',self.g_logits0)
        print('self.targets',self.targets)

        self.context_vector_seq_teach =tf.reduce_sum(self.context_vector_seq_teach0, axis=1)# batch_siz, 700, batch_len_input
        print('self.g_logits0',self.g_logits0)
...
# ezafe konim
def cal_avg_context_vector(length,len_generated_outputs, context_vector_seq, attention_weight_seqs):
...
def seq_embeddings(model, sess, args, vocab, data0,data1):
    ...
    for batch in batches:
        ...
        print(len_generated_seqs)
        print(gen_tokens[0])

        avg_context_vector ,attention_weights_masked ,attention_weight_seqs,trg_mask = cal_avg_context_vector(length,len_generated_seqs,context_vector_seq, attention_weights_seq)

        print("len(context_vector[0][:half], len(context_vector[0][:half]",avg_context_vector.shape,attention_weights_masked.shape) # 64*500
        print(',attention_weight_seqs',attention_weight_seqs[0][25])
        print('attention_weights_masked',attention_weights_masked.eval()[0][25])
    ...
    print(avg_context_vector[0, :])
    print(sen0[0])
    print(sen1[1])
    print("len(embed_data1),len(embed_data0), len(sen0), len(sen1) after loop",len(embed_data1),len(embed_data0), len(sen0), len(sen1) )
    return embed_data0, embed_data1, sen0, sen1, attention_w0, attention_w1,
    real_len_input[:half], real_len_input[half:],len_generated_seqs[:half], len_generated_seqs[half:]
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        z_seqs0, z_seqs1, sen0, sen1, att_w0,att_w1,real_len_input_0, real_len_input_1,len_generated_seqs_0, len_generated_seqs_1 = seq_embeddings(model, sess,
             args, vocab, data0, data1)
