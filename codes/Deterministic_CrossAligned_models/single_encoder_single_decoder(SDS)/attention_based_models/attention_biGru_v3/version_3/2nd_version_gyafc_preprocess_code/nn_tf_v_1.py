...
def softsample_word(dropout, proj_W, proj_b, embedding, gamma):
    def loop_func(output,t,length,end):
...
def teach_h_word(dropout, proj_W, proj_b, embedding,dec_inputs):
    def loop_func(output,t,length,end):
        ...
        if t<length:
            ...
            #inp = tf.nn.embedding_lookup(embedding, 'end')
            inp = end
        return  inp,logits
...
def attention(dim_h,batch_size,W_score1,W_score2,W_score3,output,weights_input,batch_len_input,src_context):
    ...
    print('score_h',score_h)#score_h.shape: [batch_size* batch_len_input, 1],
    ...
    scores = tf.reshape(scores,[ batch_size,  batch_len_input])# batchsize, 30
    ...
    return att, tf.reshape(context_vector,[-1,700])


def rnn_decode(mode,z,dim_h,batch_size,W_score1,W_score2,W_score3,src_context,h, inp, end, weights_input,batch_len_input,length, cell, loop_func ,scope):
    h_seq, logits_seq, context_vector_seq, attention_weights_seq = [], [], [], []
    inp_shape = inp.shape
        ...
        if mode=='':
            ...
            for t in range(length):
                ...
                if t==0:
                    ...
                    attention_weights,context_vector0 = attention(dim_h,batch_size,W_score1,W_score2,W_score3,z,weights_input,batch_len_input,src_context)
                else:
                    attention_weights,context_vector0 = attention(dim_h,batch_size,W_score1,W_score2,W_score3,h,weights_input,batch_len_input,src_context)
                ...
                print(mode,'t',t)
                print('inp',inp)
                inp,logits = loop_func(output,t,length,end)
                print('inp2',inp)
                inp.set_shape(inp_shape)
                logits_seq.append(tf.expand_dims(logits, 1))
                context_vector_seq.append(tf.expand_dims(context_vector, 1))
                # tf.expand_dims(context_vector, 1): batch_size, 1 , attention_size
                attention_weights_seq.append(tf.expand_dims(attention_weights, 1))
                # tf.expand_dims(attention_weights, 1): batch_size, 1, batch_len_input
            ...
            print(attention_weights_seq)
            print(tf.concat(attention_weights_seq,1))  #shape=(?, 31, ?), dtype=float32
            ...
            for t in range(length+1):
                h_seq.append(tf.expand_dims(h, 1))
                if t==0:
                    attention_weights,context_vector = attention(dim_h,batch_size,W_score1,W_score2,W_score3,z,weights_input,batch_len_input,src_context)
                else:
                    attention_weights,context_vector = attention(dim_h,batch_size,W_score1,W_score2,W_score3,h,weights_input,batch_len_input,src_context)
                ...
                context_vector_seq.append(tf.expand_dims(context_vector, 1))# 
                attention_weights_seq.append(tf.expand_dims(attention_weights, 1))
        #average_context_vector ,attention_weights_masked  = avg_context_vector(legnth,len_generated_seqs, tf.concat(context_vector_seq,1), tf.concat(attention_weights_seq,1))

    #real_len_input.append(real_input_seq_len)
    #attention_weights_seq=[batchsize,num_output_tokens (30 or 20),num_input_tokens_masked (less or equal to 20 or 30), 1 ]
    # context_vector_seq=[batchsize,num_output_tokens (30 or 20), 700 ]

    return tf.concat(h_seq, 1), tf.concat(logits_seq, 1), tf.concat(context_vector_seq,1), tf.concat(attention_weights_seq,1)
    # average_context_vector , attention_weights_masked , len_generated_seqs
