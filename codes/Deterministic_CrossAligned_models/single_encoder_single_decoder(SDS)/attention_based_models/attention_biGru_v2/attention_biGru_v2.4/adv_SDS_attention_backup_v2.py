...
class Model(object):
    def __init__(self, args, vocab):
        ...
        self.weights_input = tf.placeholder(tf.float32, [None, None],
            name='weights_input')
        self.dec_input_token = tf.placeholder(tf.int32, [None, None],
            name='dec_inputs')
        self.teach_h = tf.placeholder(tf.float32, [None,None,700],
            name='teach_h')
        self.g_logits = tf.placeholder(tf.float32, [ None,None],
            name='logits')     
        ...
        self.z2 = tf.concat([linear(labels, dim_y,
            scope='generator', reuse=True), self.z1 ], 1)

        cell_g = create_cell(dim_h, n_layers, self.dropout)

        self.rnn_decode = rnn_decode(embedding,self.dropout, proj_W, proj_b,W_score1,b_score1,W_score2,cell_g , scope='generator')

        #go0 = self.dec_inputs[:,0]
        #go.shape= batch_size,100

        #proj_func = teach_h_word(self.dropout, proj_W, proj_b,embedding,self.dec_input_token)
        ...
        '''
        teach_h0,_,self.output_prev= self.rnn_decode('teacher_force',self.z2,dim_h,self.batch_size,self.src_contexts,self.h_ori,go0, self.weights_input,self.batch_len_input,max_len,
            1)

        self.teach_h0,self.g_logits0, self.output = self.rnn_decode('teacher_force',self.output_prev,dim_h,self.batch_size,W_score1,b_score1,W_score2,self.src_contexts,self.h_ori,self.dec_input_token , self.weights_input,self.batch_len_input,max_len,
            1)
        self.teach_h = tf.concat(tf.transpose(self.h_seq),1)
        g_logits = tf.transpose(self.g_logits,  perm = [1, 0, 2,3])
        teach_h = tf.transpose(self.teach_h,  perm = [1, 0, 2,3])
        '''
        print('self.targets',self.targets)#('self.targets', <tf.Tensor 'targets:0' shape=(?, ?) dtype=int32>)

        print('self.g_logits.shape',self.g_logits.shape)
        ...
        soft_h_ori, soft_logits_ori,_ = self.rnn_decode('h_ori',self.z2,dim_h,self.batch_size,self.src_contexts,self.h_ori, go, self.weights_input,self.batch_len_input,max_len,
            soft_func)
        # x_fake
        soft_h_tsf, soft_logits_tsf,_  = self.rnn_decode('h_tsf',self.z2,dim_h,self.batch_size,self.src_contexts,self.h_tsf, go, self.weights_input,self.batch_len_input,max_len,
           soft_func)
        hard_h_ori, self.hard_logits_ori,_  = self.rnn_decode('h_ori',self.z2,dim_h,self.batch_size,self.src_contexts,self.h_ori, go, self.weights_input,self.batch_len_input,max_len,
            hard_func)
        hard_h_tsf, self.hard_logits_tsf,_  = self.rnn_decode('h_tsf',self.z2,dim_h,self.batch_size,self.src_contexts,self.h_tsf, go,self.weights_input,self.batch_len_input, max_len,
             hard_func)
        ...
        self.loss_d0, loss_g0 = discriminator(self.teach_h[:half], soft_h_tsf[half:],
            ones, zeros, filter_sizes, n_filters, self.dropout,
            scope='discriminator0')
        self.loss_d1, loss_g1 = discriminator(self.teach_h[half:], soft_h_tsf[:half],
            ones, zeros, filter_sizes, n_filters, self.dropout,
        ...
        theta_eg = retrive_var(['encoder', 'generator', 'embedding', 'projection', 'attention'])
        ...
        print('self.grad_rec_norm',self.grad_rec_norm)
        self.optimize_rec = opt.minimize(self.loss_rec, var_list=theta_eg)
       
        print(self.optimize_rec)
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.train:
            ...
            for epoch in range(1, 1+args.max_epochs):
                ...
                for batch in batches:
                    feed_dict = feed_dictionary(model,0, batch, rho, gamma,
                        dropout, learning_rate)
                    output,inp,batch_size,src_contexts,h,weights_input,batch_len_input=sess.run([model.z2,model.dec_input_token,model.batch_size,model.src_contexts,model.h_ori,model.weights_input,model.batch_len_input],feed_dict=feed_dict)
                    teach_h_seq, logits_seqs, output_seq = [], [], []
                    teach_h_seq.append(tf.expand_dims(h, 1))
                    for t in range(1,batch['dec_inputs'].shape[1]):
                        h_t_1,logits,output_t_1=sess.run([model.rnn_decode('teacher_force',output,args.dim_y+args.dim_z,batch_size,src_contexts,h,inp,weights_input,batch_len_input,args.max_len_length,1)],feed_dict=feed_dict)
                        feed_dict = feed_dictionary(model,t, batch, rho, gamma,
                        dropout, learning_rate)
                        teach_h_seq.append(tf.expand_dims(h_t_1, 1))
                        logits_seqs.append(tf.expand_dims(logits, 1)) # batch_size*1*vocabsize
                        h=h_t_1
                        output=output_t_1
                        #inp= sess.run([model.dec_input_token],feed_dict={ model.dec_input_token: batch['dec_inputs'][t]})
                        inp= sess.run([model.dec_input_token],feed_dict =feed_dict)
                        #h_t,logits,output_t =sess.run([model.teach_h,model.g_logits],feed_dict=feed_dict)
                    teach_h_seq_concat= tf.concat(teach_h_seq, 1)
                    logits_seqs_concat= tf.concat(logits_seqs,1)#batch_size*batch_len*vocabsize
                    _,_=sess.run([model.teach_h],feed_dict={self.teach_h:teach_h_seq_concat,self.g_logits:logits_seqs_concat})

                    feed_dict = feed_dictionary(model,0, batch, rho, gamma, dropout, learning_rate)
