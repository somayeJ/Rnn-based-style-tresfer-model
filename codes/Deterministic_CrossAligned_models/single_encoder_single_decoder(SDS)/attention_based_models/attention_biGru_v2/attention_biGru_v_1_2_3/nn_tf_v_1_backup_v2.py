...
def attention(dim_h,batch_size,W_score1,b_score1,W_score2,output,weights_input,batch_len_input,src_context):
    ...
    score_input = tf.concat([
            ...
            tf.reshape(src_context, [ batch_size, batch_len_input, 2*250 ])], axis=2)
    ...
    scores_h = tf.tanh(tf.matmul(tf.reshape(score_input, [ batch_size*batch_len_input, dim_h+2*250 ]), W_score1) + b_score1)
    ...
    expanded_att = tf.tile(tf.reshape(att, [ batch_size, batch_len_input, 1]), [1,   1, 2*250 ])
...
#teach_h,g_logits = rnn_decode(self.z0,dim_h,batch_size,W_score1,b_score1,W_score2,self.src_contexts,self.h_ori,go, self.weights_input,self.batch_len_input,self.batch_len,
    #cell_g,proj_func,scope='generator')

def rnn_decode(embedding,dropout, proj_W, proj_b,W_score1,b_score1,W_score2,cell,scope):
    def decode(mode,output_t_1,dim_h,batch_size,src_context,h,inp,weights_input,batch_len_input,length, loop_func):
        h_seq, logits_seq = [], []
        inp_shape = inp.shape
        ...
        with tf.variable_scope(scope):
            ...
            if mode=='h_ori':
                ...
                return tf.concat(h_seq, 1), tf.concat(logits_seq, 1),output
            elif mode=='h_tsf':
                print('h_tsf')
                for t in range(length):
                    h_seq.append(tf.expand_dims(h, 1))
                ...
                print("teacher_force")
                #for t in range(int(dec_inputs.shape[1])):
                #h_seq.append(tf.expand_dims(h, 1))
                output= output_t_1
                context_vector = attention(dim_h,batch_size,W_score1,b_score1,W_score2,output,weights_input,batch_len_input,src_context)
                #x = tf.concat([tf.expand_dims(context_vector, 1),inp], axis=-1)
                print('context_vector',context_vector)
                print('context_vector',context_vector.shape)# <tf.Tensor 'generator_3/Sum:0' shape=(?, ?) dtype=float32>)
                inp_emb= tf.nn.embedding_lookup(embedding, inp)
                x = tf.reshape(tf.concat([context_vector,inp_emb], axis=1),[batch_size,600])
                output, h = cell(x, h)
                #teach_h_word function here
                output = tf.nn.dropout(output, dropout)# batch_size*700
                print('output',output.shape)
                output = tf.reshape(output, [-1, dim_h])
                print('output',output.shape)
                logits = tf.matmul(output, proj_W) + proj_b # batch_size*vocabsize

                #inp,logits = loop_func(output)
                #inp.set_shape(inp_shape)
                #logits_seq.append(tf.expand_dims(logits, 1))
                #return tf.concat(h_seq, 1), tf.concat(logits_seq, 1), output
                return h, logits, output
    return decode

def attention1(dim_h,batch_size,W_score1,b_score1,W_score2,output,weights_input,batch_len_input,src_context):
    #src_contexts.shape: [batchsize,1, batch_len_input, 2*250]
...
#soft_h_tsf, soft_logits_tsf = rnn_decode(self.h_tsf, go, max_len,
            #cell_g, soft_func, scope='generator')   
def rnn_decode1(dim_h,batch_size,W_score1,b_score1,W_score2,src_context,h, inp, weights_input,batch_len_input,length, cell, loop_func, scope):
