...
def rnn_decode(mode,dec_inputs,z,dim_h,batch_size,W_score1,b_score1,W_score2,src_context,h, inp,  weights_input,batch_len_input,length, cell, loop_func, scope):
    h_seq, logits_seq = [], []
    inp_shape = inp.shape
    ...
    with tf.variable_scope(scope):
        ...
        if mode=='':
            ...
            #iter=tf.data.Iterator(dec_inputs)
            #optional = iterator.get_next_as_optional()
            #if optional.has_value():
            #for t in tf.range((dec_inputs[0,:,0])):
            for t in dec_inputs[0,:,0]:
                h_seq.append(tf.expand_dims(h, 1))
                if t==0:
                    output= z
                t+=1
