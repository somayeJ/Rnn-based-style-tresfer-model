...
def attention(dim_h,batch_size,W_score1,b_score1,W_score2,output,weights_input,batch_len_input,src_context):
    ...
    scores_h = tf.tanh(tf.matmul(tf.reshape(score_input, [ batch_size*batch_len_input, dim_h+2*350 ]), W_score1) + b_score1)
    ...
    #version ghabli:scores *= tf.reshape(src_mask,[-1])
...
def rnn_decode(mode,z,dim_h,batch_size,W_score1,b_score1,W_score2,src_context,h, inp, end, weights_input,batch_len_input,length, cell, loop_func, scope):
    ...
    with tf.variable_scope(scope):
        ...
        if mode=='':
            ...
            for t in range(length):
                ...
                attention_weights,context_vector0 = attention(dim_h,batch_size,W_score1,b_score1,W_score2,output,weights_input,batch_len_input,src_context)
            ...
            for t in range(length+1):
                ...
                attention_weights,context_vector = attention(dim_h,batch_size,W_score1,b_score1,W_score2,output,weights_input,batch_len_input,src_context)
                print('inp',inp)
                print('output',output)
                print(mode,t,'t')
