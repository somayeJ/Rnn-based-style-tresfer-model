    ...
    def __init__(self, sess, args, vocab, model):
        ...
        with tf.variable_scope('generator'):
            ...
            score_input = tf.concat([
                tf.reshape(expanded_tgt, [ self.batch_size, self.batch_len_input, dim_h ]),
                tf.reshape(self.src_contexts, [ self.batch_size, self.batch_len_input, 2*350 ])], axis=2)

            scores_h = tf.tanh(tf.matmul(tf.reshape(score_input, [ self.batch_size*self.batch_len_input, dim_h+2*350 ]), W_score1) + b_score1)
            scores   = tf.reshape(tf.matmul(scores_h, W_score2), [ self.batch_size* self.batch_len_input ]) 
            ...
            context_vector = tf.reduce_sum(self.src_contexts*expanded_att, axis=1)
