# this is made based on style_transfer.py just to have AE part, 1 layer GRU AE, and now the sizes are based on elmo
# representation
import os
import sys
import time
...
def transfer(model, decoder, sess, args, vocab, data0, data1, dev0_full, dev1_full, out_path):
    ...
    for batch in batches:
        ...
        print('siiiiiiiiiiiiiiiize', half, batch['size'])
        data0_rec += rec[:half]
        data1_rec += rec[half:]
        data0_tsf += tsf[:half]
        data1_tsf += tsf[half:]
        ...
        loss_rec = sess.run([model.loss_rec],
            feed_dict=feed_dictionary(model, batch,  args.extra_features, args.rho, args.gamma_min))
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.beam > 1:
            ...
            decoder = beam_search.Decoder(sess, args, vocab, model, model.embedding)
        else:
