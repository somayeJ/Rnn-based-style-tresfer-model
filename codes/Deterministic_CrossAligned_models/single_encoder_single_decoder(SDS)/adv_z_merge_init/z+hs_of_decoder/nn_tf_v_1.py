...
def feed_forward_h(h, dropout, scope):
    #dim = inp.get_shape().as_list()[-1]
    dim = h.shape.as_list()[-1]
    dim1 = 700 # dim_z
    ...
    with tf.variable_scope(tf.get_variable_scope(),reuse=tf.AUTO_REUSE) as vs:
        ...
        #h1 = leaky_relu(tf.matmul(inp, W1) + b1)
        print('W3',W3)
    logits = tf.matmul(h, W3) + b3
    return logits

def rnn_decode(dropout, z,h, inp, length, cell, loop_func, scope):
    h_seq, logits_seq = [], []
    inp_shape = inp.shape
    ...
    with tf.variable_scope(scope):
        ...
        for t in range(length):
            ...
            print(h,'*'*120)
            print( 'innnnnnnnnnnnnp',inp)
            output, h0 = cell(inp, h) # h0 size = dim_z *2 + dim_y: 1200

            h =  tf.math.add(h0,z, name='hplusz')
