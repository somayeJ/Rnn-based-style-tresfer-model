...
class Model(object):
    ...
    def __init__(self, args, vocab):
        ...
        self.embedding =  tf.concat([embedding,
            tf.zeros([embedding.shape[0], dim_z])], 1)
        ...
        with tf.variable_scope('projection_input'):
            proj_W_inp = tf.get_variable('W_input', [dim_z+dim_emb, dim_emb])
            proj_b_inp = tf.get_variable('b_input', [dim_emb])
        ...
        dec_inputs = tf.concat([dec_inputs[:,:,:100], tf.tile(tf.reshape(self.z, [self.batch_size, 1, dim_z]), [1, self.batch_len, 1])], 2)
        ...
        soft_h_ori, soft_logits_ori = rnn_decode( self.z,self.h_ori, go, max_len,
            cell_g, soft_func, scope='generator')
        # x_fake
        soft_h_tsf, soft_logits_tsf = rnn_decode( self.z,self.h_tsf, go, max_len,
            cell_g, soft_func, scope='generator')
        hard_h_ori, self.hard_logits_ori = rnn_decode( self.z,self.h_ori, go, max_len,
            cell_g, hard_func, scope='generator')
        hard_h_tsf, self.hard_logits_tsf = rnn_decode( self.z,self.h_tsf, go, max_len,
            cell_g, hard_func, scope='generator')
...
def transfer(model, decoder, sess, args, vocab, data0, data1, out_path):
    ...
    for batch in batches:
        #decoder.rewrite(batch): hard logits ro tabdil be kalame mikone va az token end be baed hazf mikone
