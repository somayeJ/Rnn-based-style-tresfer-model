...
def feed_forward_inp(inp, inp_shape, scope):
    #dim = inp.get_shape().as_list()[-1]
    dim = inp_shape.as_list()[-1]
    dim1 = 100
    with tf.variable_scope(tf.get_variable_scope(),reuse=tf.AUTO_REUSE) as vs:
        #tf.get_variable_scope().reuse_variables()

        #if reuse:
         #   print('reeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeuse',reuse)
         #   vs.reuse_variables()


        W3 = tf.get_variable('W3', [dim, dim1])
        b3 = tf.get_variable('b3', [dim1])
        #print(55555555555555555555555555555555555555555555555555555555555555555,W3, b3)
        #W2 = tf.get_variable('W2', [dim, 1])
        #b2 = tf.get_variable('b2', [1])
        #h1 = leaky_relu(tf.matmul(inp, W1) + b1)
        print('W3',W3)
    logits = tf.matmul(inp, W3) + b3

    return logits
def rnn_decode(z,h, inp, length, cell, loop_func, scope):
    h_seq, logits_seq = [], []
    inp_shape = inp.shape
    inp = tf.concat([inp[:,:100], z], 1)

    print('iiiiiiiiiinnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnp',z,inp_shape,inp)
    with tf.variable_scope(scope):
        tf.get_variable_scope().reuse_variables()
        for t in range(length):
            h_seq.append(tf.expand_dims(h, 1))
            print('iiiiiiiiiinnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnp', h,inp)

            output, h = cell(inp, h)
            print('ippppppppppppppppppppppppppppppppppppppppnp', h,output)

            inp0, logits = loop_func(output) # inp : size:600
            inp1=feed_forward_inp(inp0,inp_shape,scope)
            print('11111111111111111111111111', inp1)

            inp = tf.concat([inp1, z], 1) # inp
            inp.set_shape(inp_shape)
            print('iiiiiiiiiinnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnp', logits,inp)
