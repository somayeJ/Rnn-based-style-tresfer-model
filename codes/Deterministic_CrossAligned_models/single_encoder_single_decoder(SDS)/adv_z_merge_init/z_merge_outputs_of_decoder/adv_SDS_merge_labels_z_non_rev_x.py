...
from utils_tf_1_labels_z_non_rev_x import *
from nn_tf_v_1_labels_z import *
import beam_search_tf_1_labels_z, greedy_decoding_tf_1
...
class Model(object):
    ...
    def __init__(self, args, vocab):
        ...
        with tf.variable_scope('projection'):
            proj_W = tf.get_variable('W', [dim_h +dim_h , vocab.size])
        ...
        #print('111',g_outputs)
        #print('222',tf.tile(tf.reshape(self.z, [self.batch_size, 1, dim_z]), [1, max_len, 1]))

        #g_outputs = tf.concat([g_outputs0, self.z],1)
        #print('g_outputs',g_outputs) # Tensor("generator_2/transpose_1:0", shape=(?, ?, 700), dtype=float32) # shape= batch_size* max_len + 1* dim_h
        ...
        #teach_h = tf.concat([tf.expand_dims(tf.concat([self.h_ori,self.z],1), 1), g_outputs], 1)
        #teach_h = tf.concat([tf.expand_dims(tf.concat([self.h_ori, g_outputs], 1)
        ...
        g_outputs = tf.concat([g_outputs0, tf.tile(tf.reshape(self.h_ori, [self.batch_size, 1, dim_h]), [1, self.len, 1])],  axis=2)  # in reshaping the product of the dimensions (SIZE) must remain the same as that of the original shape

        g_outputs = tf.reshape(g_outputs, [-1, dim_h+dim_h])
        ...
        soft_func_ori = softsample_word(self.h_ori,self.dropout, proj_W, proj_b, self.embedding,
            self.gamma)
        soft_func_tsf = softsample_word(self.h_tsf,self.dropout, proj_W, proj_b, self.embedding,
            self.gamma)
        ...
        hard_func_ori = argmax_word(self.h_ori,self.dropout, proj_W, proj_b, self.embedding)
        hard_func_h_tsf = argmax_word(self.h_tsf,self.dropout, proj_W, proj_b, self.embedding)
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.beam > 1:
            decoder = beam_search_tf_1_labels_z.Decoder(sess, args, vocab, model)
        else:
