...
class Decoder(object):
    ...
    def __init__(self, sess, args, vocab, model):
        ...
        self.labels_h =tf.placeholder(tf.float32, [None, dim_h])
        ...
        with tf.variable_scope('generator'):
            ...
            outputs = tf.concat([outputs0,self.labels_h],1)
        ...
        for t in range(self.max_len):
            ...
            for state in beam:
                log_lh, indices, h = self.sess.run(
                    ...
                    feed_dict={self.inp: state.inp, self.h: state.h, self.labels_h:labels_h })
    ...
    def rewrite(self, batch):
        ...
        ori = self.decode(h_ori,h_ori)
        ori = [[self.vocab.id2word[i] for i in sent] for sent in ori]
        ori = strip_eos(ori)
        tsf = self.decode(h_tsf,h_tsf)
