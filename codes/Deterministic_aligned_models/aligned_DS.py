# this code is the same as style_transfer.py, but just z1 & z2 are aligned
import os
import sys
import time
import ipdb
...
#import beam_search_h0, greedy_decoding
import beam_search_h0, greedy_decoding_tf_1
...
class Model(object):
    ...
    def __init__(self, args, vocab):
        ...
        teach_h = tf.expand_dims(self.h_ori, 1)
        soft_h_tsf_h0 = tf.expand_dims(self.h_tsf, 1)
        ...
        # we produce to the no of max_len the size can be produced after the token ends (they are not pads at the end)
        print(teach_h[:half])

        self.loss_d0, loss_g0 = discriminator_aligned(teach_h[:half], soft_h_tsf_h0[half:],
            ones, zeros, filter_sizes, n_filters, self.dropout,
            scope='discriminator0')
        self.loss_d1, loss_g1 = discriminator_aligned(teach_h[half:], soft_h_tsf_h0[:half],
            ones, zeros, filter_sizes, n_filters, self.dropout,
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.beam > 1:
            decoder = beam_search_h0.Decoder(sess, args, vocab, model)
        else:
